...
import os
import os.path
from utils.common import get_my_logger, is_windows
...
class BaseInterface(object):
    ...
    def __init__(self, working_dir=None):
        self.working_dir = working_dir if working_dir else os.environ.get('WS_HOME', os.getcwd())
        self.loader = None
...
class LoaderInterface(BaseInterface):
    ...
    def __check_config_file(self, config_file):  #was __parse__config
        ...
        if not os.path.isfile(config_file):
            ...
            if not os.path.isfile(config_file):
                raise OSError(2, "No such file ", config_file)
        return config_file

    def run(self, config_file, host_list=None, dryrun=None):
        ''' from config file '''
        assert self.loader, "use_database first"
        assert os.path.exists(config_file), 'invalid config file %s' % config_file
