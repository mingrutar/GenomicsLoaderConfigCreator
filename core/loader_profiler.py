...
class LoaderProfiler(ProfilerBase):
    def __init__(self, data_path):
        super(self.__class__, self).__init__(data_path)
    ...
    def dry_run(self, jsondef_list, host_list=None):
        ''' dry run, not call run_exec '''
        run_list = []
        if isinstance(jsondef_list, list):
            for jsondef in jsondef_list:
                run_list.append(self.__prepare_test(jsondef))
        else:
            run_list = self.__prepare_test(jsondef_list)
